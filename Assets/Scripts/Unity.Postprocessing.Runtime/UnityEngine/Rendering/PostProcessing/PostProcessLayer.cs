...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	public sealed class PostProcessLayer : MonoBehaviour
	{
		...
		private void OnPreCull()
		{
			...
			if (m_CurrentContext.IsTemporalAntialiasingActive() && !m_Camera.usePhysicalProperties)
			{
		...
		private int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1, int eye = -1)
		{
			...
			{
				command.BlitFullscreenTriangle(context.source, context.destination, propertySheet, 0, clear: false, context.camera.pixelRect);
			}
			else
			{
		...
		private void RenderFinalPass(PostProcessRenderContext context, int releaseTargetAfterUse = -1, int eye = -1)
		{
			...
			else
			{
				...
				{
					command.BlitFullscreenTriangle(context.source, context.destination, propertySheet2, 0, clear: false, context.camera.pixelRect);
				}
				if (num > -1)
				{
					command.ReleaseTemporaryRT(num);
		...
		{
			if (format != 0 && format != RenderTextureFormat.ARGBHalf && format != RenderTextureFormat.ARGB4444 && format != RenderTextureFormat.ARGB1555 && format != RenderTextureFormat.ARGB2101010 && format != RenderTextureFormat.ARGB64 && format != RenderTextureFormat.ARGBFloat && format != RenderTextureFormat.ARGBInt && format != RenderTextureFormat.BGRA32 && format != RenderTextureFormat.RGBAUShort)
			{
				return format == RenderTextureFormat.BGRA10101010_XR;
			}
			return true;
		}
	}
