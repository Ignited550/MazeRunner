	...
	internal sealed class Plane
	{
		...
		public void SplitPolygon(Polygon polygon, List<Polygon> coplanarFront, List<Polygon> coplanarBack, List<Polygon> front, List<Polygon> back)
		{
			...
			for (int i = 0; i < polygon.vertices.Count; i++)
			{
				...
				EPolygonType ePolygonType2 = ((num < -1E-05f) ? EPolygonType.Back : ((num > 1E-05f) ? EPolygonType.Front : EPolygonType.Coplanar));
			...
			case EPolygonType.Spanning:
			{
				...
				if (list2.Count >= 3)
				{
					front.Add(new Polygon(list2, polygon.material));
				}
				if (list3.Count >= 3)
				{
					back.Add(new Polygon(list3, polygon.material));
				}
				break;
			}
			}
