...
namespace UnityEngine.ProBuilder
{
	public static class Math
	{
		...
		{
			Vector2 from = lineEnd - lineStart;
			Vector2 vector = new Vector2(0f - from.y, from.x);
			float num = Mathf.Sin(Vector2.Angle(from, point - lineStart) * ((float)System.Math.PI / 180f)) * Vector2.Distance(point, lineStart);
			return point + vector * (num * 2f) * ((Vector2.Dot(point - lineStart, vector) > 0f) ? (-1f) : 1f);
		}
		...
		public static Normal NormalTangentBitangent(ProBuilderMesh mesh, Face face)
		{
			...
			Normal result = default(Normal);
			result.normal = vector;
			result.tangent = vector2;
			result.bitangent = Vector3.Cross(vector, (Vector3)vector2 * vector2.w);
			return result;
		}
		...
		internal static Bounds GetBounds(Vector3[] positions, IEnumerable<Face> faces)
		{
			...
			foreach (Face face in faces)
			{
				...
				{
					flag = true;
					vector = positions[distinctIndexesInternal[0]];
					vector2 = positions[distinctIndexesInternal[0]];
				}
				int i = 0;
				for (int num = distinctIndexesInternal.Length; i < num; i++)
				{
					vector.x = Mathf.Min(positions[distinctIndexesInternal[i]].x, vector.x);
					vector2.x = Mathf.Max(positions[distinctIndexesInternal[i]].x, vector2.x);
					vector.y = Mathf.Min(positions[distinctIndexesInternal[i]].y, vector.y);
					vector2.y = Mathf.Max(positions[distinctIndexesInternal[i]].y, vector2.y);
					vector.z = Mathf.Min(positions[distinctIndexesInternal[i]].z, vector.z);
					vector2.z = Mathf.Max(positions[distinctIndexesInternal[i]].z, vector2.z);
				}
			}
			return new Bounds((vector + vector2) * 0.5f, vector2 - vector);
		}
		...
		internal static Bounds GetBounds(Vector3[] positions, IEnumerable<Edge> edges)
		{
			...
			foreach (Edge edge in edges)
			{
				...
				{
					flag = true;
					vector = positions[edge.a];
					vector2 = positions[edge.a];
				}
				vector = ComponentMin(positions[edge.a], vector);
				vector2 = ComponentMax(positions[edge.a], vector2);
				vector = ComponentMin(positions[edge.b], vector);
				vector2 = ComponentMax(positions[edge.b], vector2);
			}
			return new Bounds((vector + vector2) * 0.5f, vector2 - vector);
		}
