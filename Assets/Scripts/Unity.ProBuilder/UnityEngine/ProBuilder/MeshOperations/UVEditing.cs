...
namespace UnityEngine.ProBuilder.MeshOperations
{
	internal static class UVEditing
	{
		...
		{
			Vector2 vector = dst[GetIndex(dstIndices, 1)] - dst[GetIndex(dstIndices, 0)];
			Vector2 vector2 = src[GetIndex(srcIndices, 1)] - src[GetIndex(srcIndices, 0)];
			float num = Vector2.Angle(vector, vector2);
			if (Vector2.Dot(Vector2.Perpendicular(vector), vector2) < 0f)
			{
				num = 360f - num;
			}
			...
			result.translation = vector3 - bounds2D.center;
			result.rotation = num;
		...
		public static bool AutoStitch(ProBuilderMesh mesh, Face f1, Face f2, int channel)
		{
			...
			if (AlignEdges(mesh, f2, wingedEdge.edge.local, wingedEdge.opposite.edge.local, channel))
			{
				if (!f2.manualUV)
				{
					SetAutoAndAlignUnwrapParamsToUVs(mesh, new Face[1] { f2 });
				}
				return true;
			}
			return false;
			...
			for (int i = 0; i < indexes.Length - 1; i++)
			{
				for (int j = i + 1; j < indexes.Length; j++)
				{
					...
					if (value != b && Vector2.Distance(array[indexes[i]], array[indexes[j]]) < delta)
					{
						...
						int[] array2 = (from x in sharedTextureLookup
							where x.Value == b
							select x into y
		...
		public static Vector2[] FitUVs(Vector2[] uvs)
		{
			...
			float num = Math.LargestValue(Math.LargestVector2(uvs));
			for (int i = 0; i < uvs.Length; i++)
			{
				uvs[i] /= num;
			}
