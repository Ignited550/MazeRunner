	...
	public sealed class ProBuilderMesh : MonoBehaviour
	{
		...
		public MeshSyncState meshSyncState
		{
			get
			{
				...
				int.TryParse(mesh.name.Replace("pb_Mesh", ""), out var result);
				if (result != id)
				{
		...
		public Vertex[] GetVertices(IList<int> indexes = null)
		{
			...
			GetUVs(2, list);
			GetUVs(3, list2);
			bool flag = array2 != null && array2.Count() == num;
			bool flag2 = array3 != null && array3.Count() == num;
			bool flag3 = array6 != null && array6.Count() == num;
			bool flag4 = array5 != null && array5.Count() == num;
			bool flag5 = array4 != null && array4.Count() == num;
			bool flag6 = array7 != null && array7.Count() == num;
		...
		private void Reset()
		{
			if (meshSyncState != 0 && meshSyncState != MeshSyncState.InstanceIDMismatch)
			{
				Rebuild();
		...
		internal static ProBuilderMesh CreateInstanceWithPoints(Vector3[] positions)
		{
			...
			proBuilderMesh.m_MeshFormatVersion = 1;
		...
		public void ToMesh(MeshTopology preferredTopology = MeshTopology.Triangles)
		{
			...
			{
				mesh.SetIndices(submeshes[i].m_Indexes, submeshes[i].m_Topology, i, calculateBounds: false);
			}
		...
		{
			if (base.gameObject.TryGetComponent<MeshCollider>(out var component) && component.sharedMesh != mesh)
			{
		...
		private void CacheSelection()
		{
			...
			try
			{
				...
				foreach (int key in array)
				{
					...
					foreach (int item in sharedVertex)
					{
						m_SelectedCoincidentVertices.Add(item);
					}
				}
			}
			catch
