...
namespace UnityEngine.ProBuilder
{
	public static class Projection
	{
		...
		internal static void PlanarProject(IList<Vector3> positions, IList<int> indexes, Vector3 direction, List<Vector2> results)
		{
			...
			Vector3 tangentToAxis = GetTangentToAxis(VectorToProjectionAxis(direction));
			int num = indexes?.Count ?? positions.Count;
			results.Clear();
			Vector3 lhs = Vector3.Cross(direction, tangentToAxis);
			Vector3 lhs2 = Vector3.Cross(lhs, direction);
			lhs.Normalize();
			lhs2.Normalize();
			if (indexes != null)
			{
