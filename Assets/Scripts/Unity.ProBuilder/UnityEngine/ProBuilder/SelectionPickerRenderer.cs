	...
	internal static class SelectionPickerRenderer
	{
		...
		public static Dictionary<ProBuilderMesh, HashSet<int>> PickVerticesInRect(Camera camera, Rect pickerRect, IList<ProBuilderMesh> selection, bool doDepthTest, int renderTextureWidth = -1, int renderTextureHeight = -1)
		{
			...
			foreach (KeyValuePair<ProBuilderMesh, HashSet<int>> item in dictionary)
			{
				...
				int k = 0;
				for (int num6 = sharedVertices.Length; k < num6; k++)
				{
		...
		private static GameObject[] GenerateFacePickingObjects(IList<ProBuilderMesh> selection, out Dictionary<uint, SimpleTuple<ProBuilderMesh, Face>> map)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				GameObject gameObject = InternalUtility.EmptyGameObjectWithTransform(proBuilderMesh.transform);
				gameObject.name = proBuilderMesh.name + " (Face Depth Test)";
				Mesh mesh = new Mesh();
				...
				gameObject.AddComponent<MeshRenderer>().sharedMaterial = BuiltinMaterials.facePickerMaterial;
				array[i] = gameObject;
			}
			return array;
		}
		...
		private static void GenerateVertexPickingObjects(IList<ProBuilderMesh> selection, bool doDepthTest, out Dictionary<uint, SimpleTuple<ProBuilderMesh, int>> map, out GameObject[] depthObjects, out GameObject[] pickerObjects)
		{
			...
			for (int i = 0; i < count; i++)
			{
				ProBuilderMesh proBuilderMesh = selection[i];
				GameObject gameObject = InternalUtility.EmptyGameObjectWithTransform(proBuilderMesh.transform);
				gameObject.name = proBuilderMesh.name + "  (Vertex Billboards)";
				gameObject.AddComponent<MeshFilter>().sharedMesh = BuildVertexMesh(proBuilderMesh, map, ref index);
				gameObject.AddComponent<MeshRenderer>().sharedMaterial = BuiltinMaterials.vertexPickerMaterial;
			...
			if (doDepthTest)
			{
				...
				for (int j = 0; j < count; j++)
				{
					...
					GameObject gameObject2 = InternalUtility.EmptyGameObjectWithTransform(proBuilderMesh2.transform);
					gameObject2.name = proBuilderMesh2.name + "  (Depth Mask)";
					gameObject2.AddComponent<MeshFilter>().sharedMesh = proBuilderMesh2.mesh;
					gameObject2.AddComponent<MeshRenderer>().sharedMaterial = BuiltinMaterials.facePickerMaterial;
		...
		private static void GenerateEdgePickingObjects(IList<ProBuilderMesh> selection, bool doDepthTest, out Dictionary<uint, SimpleTuple<ProBuilderMesh, Edge>> map, out GameObject[] depthObjects, out GameObject[] pickerObjects)
		{
			...
			for (int i = 0; i < count; i++)
			{
				ProBuilderMesh proBuilderMesh = selection[i];
				GameObject gameObject = InternalUtility.EmptyGameObjectWithTransform(proBuilderMesh.transform);
				gameObject.name = proBuilderMesh.name + "  (Edge Billboards)";
				gameObject.AddComponent<MeshFilter>().sharedMesh = BuildEdgeMesh(proBuilderMesh, map, ref index);
				gameObject.AddComponent<MeshRenderer>().sharedMaterial = BuiltinMaterials.edgePickerMaterial;
				pickerObjects[i] = gameObject;
			}
			if (doDepthTest)
			{
