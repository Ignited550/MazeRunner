...
namespace UnityEngine.ProBuilder
{
	public static class Smoothing
	{
		...
		internal static void ApplySmoothingGroups(ProBuilderMesh mesh, IEnumerable<Face> faces, float angleThreshold, Vector3[] normals)
		{
			...
			float angleThreshold2 = Mathf.Abs(Mathf.Cos(Mathf.Clamp(angleThreshold, 0f, 89.999f) * ((float)System.Math.PI / 180f)));
			...
			foreach (WingedEdge wingedEdge in WingedEdge.GetWingedEdges(mesh, faces, oneWingPerFace: true))
			{
				if (hashSet2.Add(wingedEdge.face))
				{
					wingedEdge.face.smoothingGroup = nextUnusedSmoothingGroup;
					if (FindSoftEdgesRecursive(normals, wingedEdge, angleThreshold2, hashSet2))
					{
