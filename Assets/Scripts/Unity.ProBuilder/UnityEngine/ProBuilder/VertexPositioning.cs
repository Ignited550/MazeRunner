	...
	public static class VertexPositioning
	{
		...
		internal static void TranslateVerticesInWorldSpace(this ProBuilderMesh mesh, int[] indexes, Vector3 offset, float snapValue, bool snapAxisOnly)
		{
			...
			if (Mathf.Abs(snapValue) > Mathf.Epsilon)
			{
				...
				{
					Vector3 vertex = localToWorldMatrix.MultiplyPoint3x4(positionsInternal[s_CoincidentVertices[num]] + vector);
					positionsInternal[s_CoincidentVertices[num]] = worldToLocalMatrix.MultiplyPoint3x4(ProBuilderSnapping.SnapValue(vertex, (Vector3)vector3Mask * snapValue));
				}
			}
			else
			{
