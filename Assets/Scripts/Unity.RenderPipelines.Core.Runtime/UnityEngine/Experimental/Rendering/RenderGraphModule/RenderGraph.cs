...
namespace UnityEngine.Experimental.Rendering.RenderGraphModule
{
	public class RenderGraph
	{
			...
			for (int i = 0; i < m_CompiledPassInfos.size; i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					...
					foreach (ResourceHandle item2 in reference.pass.resourceWriteLists[j])
					{
						...
						reference.refCount++;
						if (m_Resources.IsResourceImported(in res))
						{
		...
		private ref CompiledPassInfo CompilePassImmediatly(RenderGraphPass pass)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				foreach (ResourceHandle item in pass.resourceWriteLists[i])
				{
					...
					if (!m_Resources.IsResourceCreated(in res))
					{
		...
		private void PreRenderPassExecute(in CompiledPassInfo passInfo, RenderGraphContext rgContext)
		{
			...
			foreach (int item in passInfo.resourceCreateList[0])
			{
				m_Resources.CreateAndClearTexture(rgContext, item);
			}
			foreach (int item2 in passInfo.resourceCreateList[1])
			{
				m_Resources.CreateComputeBuffer(rgContext, item2);
			}
		...
		private void PostRenderPassExecute(ref CompiledPassInfo passInfo, RenderGraphContext rgContext)
		{
			...
			foreach (int item in passInfo.resourceReleaseList[0])
			{
				m_Resources.ReleaseTexture(rgContext, item);
			}
			foreach (int item2 in passInfo.resourceReleaseList[1])
			{
				m_Resources.ReleaseComputeBuffer(rgContext, item2);
			}
		}
		...
		{
			foreach (int item in m_ImmediateModeResourceList[0])
			{
				m_Resources.ReleaseTexture(m_RenderGraphContext, item);
			}
			foreach (int item2 in m_ImmediateModeResourceList[1])
			{
				m_Resources.ReleaseComputeBuffer(m_RenderGraphContext, item2);
			}
		}
		...
		private void GenerateDebugData()
		{
			...
			for (int i = 0; i < 2; i++)
			{
				for (int j = 0; j < m_CompiledResourcesInfos[i].size; j++)
				{
					...
					data.name = m_Resources.GetResourceName((RenderGraphResourceType)i, j);
					data.imported = m_Resources.IsResourceImported((RenderGraphResourceType)i, j);
