	...
	public class TimelineClip : ICurvesOwner, ISerializationCallbackReceiver
	{
		...
		public double easeInDuration
		{
			get
			{
				...
				return Math.Min(Math.Max(m_EaseInDuration, 0.0), duration);
			}
			set
			{
				m_EaseInDuration = (clipCaps.HasAny(ClipCaps.Blending) ? Math.Max(0.0, Math.Min(SanitizeTimeValue(value, m_EaseInDuration), duration)) : 0.0);
			}
		}
		...
		public double easeOutDuration
		{
			get
			{
				...
				return Math.Min(Math.Max(m_EaseOutDuration, 0.0), duration);
			}
			set
			{
				m_EaseOutDuration = (clipCaps.HasAny(ClipCaps.Blending) ? Math.Max(0.0, Math.Min(SanitizeTimeValue(value, m_EaseOutDuration), duration)) : 0.0);
			}
		}
