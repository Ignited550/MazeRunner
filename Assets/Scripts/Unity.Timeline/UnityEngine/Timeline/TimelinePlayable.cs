...
namespace UnityEngine.Timeline
{
	public class TimelinePlayable : PlayableBehaviour
	{
		...
		private Playable CreateTrackPlayable(PlayableGraph graph, Playable timelinePlayable, TrackAsset track, GameObject go, bool createOutputs)
		{
			...
			{
				throw new InvalidOperationException(track.name + "(" + track.GetType()?.ToString() + ") did not produce a valid playable. Use the compilable property to indicate whether the track is valid for processing");
			}
